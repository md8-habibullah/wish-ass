...
export default function SellerDashboard() {
  ...
  const statCards = [
    ...
    { title: "Medicines", value: stats?.activeMedicines, icon: Pill, color: "text-primary", bg: "bg-primary/10", trend: "+2 new" },
  ...
  return (
    <div className="container px-4 py-12 md:px-8 max-w-7xl space-y-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 p-10 rounded-[48px] bg-zinc-900 text-white relative overflow-hidden shadow-2xl">
        ...
        <div className="relative z-10 flex gap-4">
           <Link href="/seller/medicines/new">
             <Button className="bg-primary hover:bg-primary h-16 px-10 rounded-3xl font-extrabold text-lg shadow-xl shadow-primary/20 transition-all hover:scale-105 active:scale-95">
            ...
            <Card className="group h-full rounded-[48px] border-white/5 shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 overflow-hidden cursor-pointer bg-card">
              <CardContent className="p-10 space-y-8">
                ...
                <div className="space-y-3">
                  ...
                  <h3 className="text-6xl font-black text-white font-heading tracking-tighter leading-none">{stat.value}</h3>
                </div>
              </CardContent>
            </Card>
          );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        {/* Recent Orders */}
        <Card className="lg:col-span-2 rounded-[48px] border-white/5 shadow-sm overflow-hidden bg-card">
          <CardHeader className="p-10 border-b border-background flex flex-row items-center justify-between">
            <div className="space-y-1">
            ...
            <Link href="/seller/orders">
              <Button variant="ghost" className="text-primary font-bold hover:bg-primary/10 rounded-2xl px-6">
                View Full Queue
                   ...
                   if (pendingOrders.length === 0) {
                      return (
                        <div className="p-20 text-center space-y-4">
                           <div className="p-6 bg-background rounded-full w-fit mx-auto">
                              <ShoppingBag className="h-10 w-10 text-zinc-200" />
                           </div>
                           <div className="space-y-2">
                              <p className="text-xl font-bold text-white font-heading">No pending tasks</p>
                   ...
                   return pendingOrders.map((order: { id: string; createdAt: string; totalPrice: string | number; status: string; user?: { name: string } }) => (
                      <div key={order.id} className="p-6 hover:bg-background/50 transition-colors flex items-center justify-between group">
                         <div className="flex items-center gap-4">
                            ...
                            <div>
                               <p className="text-sm font-bold text-white">Order #{order.id.slice(0, 8)}</p>
                               <p className="text-xs text-zinc-500">{order.user?.name} • {new Date(order.createdAt).toLocaleDateString()}</p>
                            </div>
                         </div>
                         <div className="flex items-center gap-6">
                            <div className="text-right">
                               <p className="text-sm font-bold text-white">${Number(order.totalPrice).toFixed(2)}</p>
                               <p className="text-[10px] font-bold text-primary uppercase tracking-widest">{order.status}</p>
                            </div>
                            <Link href="/seller/orders">
        ...
        {/* Low Stock Alerts */}
        <Card className="rounded-[48px] border-white/5 shadow-sm overflow-hidden">
          <CardHeader className="p-10 border-b border-background">
            <div className="space-y-1">
          ...
          <CardContent className="p-10 space-y-8">
             <div className="space-y-4">
               {[1, 2, 3].map(i => (
                 <div key={i} className="flex items-center justify-between p-5 rounded-3xl bg-background border border-white/5 group hover:border-red-200 transition-all">
                    <div className="flex items-center gap-4">
                       <div className="w-12 h-12 bg-card rounded-2xl border border-zinc-200 flex items-center justify-center p-2">
                          <Pill className="h-6 w-6 text-zinc-400 group-hover:text-red-500 transition-colors" />
                       </div>
                       <div>
                          <p className="font-bold text-white text-sm">Advanced Medication {i}</p>
                          <p className="text-xs text-zinc-500">Remaining: <span className="text-red-600 font-bold">{i * 2} units</span></p>
                       </div>
                    </div>
                    <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full hover:bg-card text-zinc-300 hover:text-white">
             ...
             <Link href="/seller/medicines">
                <Button variant="outline" className="w-full h-14 rounded-3xl border-zinc-200 font-bold hover:bg-background">
      ...
      <Card className="rounded-[48px] border-white/5 shadow-sm overflow-hidden bg-card">
         <CardHeader className="p-10 border-b border-background flex flex-row items-center justify-between">
            <div className="space-y-1">
            ...
            <Select defaultValue="30d">
               <SelectTrigger className="w-32 rounded-xl border-white/5">
               ...
               {[40, 60, 45, 80, 55, 90, 70, 85, 60, 75, 50, 65].map((val, i) => (
                 <div key={i} className="flex-1 space-y-4 flex flex-col items-center group">
                    <div className="relative w-full h-full flex items-end justify-center">
                       <div 
                         className="w-full rounded-t-xl bg-primary/10 group-hover:bg-primary transition-all duration-500" 
                         style={{ height: `${val}%` }} 
                       />
